...
struct SolveResStore {
    ...
    SolveResStore(std::vector<Tree> const &trees, std::vector<std::vector<BP_Shape>> const &shpsAlterns,
        ...
        : m_trees(trees), m_sqsz(shpsAlterns.front().front().m_height + 2), m_shpsAlterns(shpsAlterns),
          ...
          m_reaAreaMaps(std::from_range,
                        std::views::transform(trees,
                                              [this](auto const &item) {
                                                  return std::vector<std::uint8_t>((item.yDim + (2 * m_sqsz - 4)) *
                                                                                       (item.xDim + (2 * m_sqsz - 4)),
                                                                                   0);
                                              })),
    ...
    auto get_mdspan_areaTree_borderless(this auto &self, size_t const id) {
...
inline std::string file_toString(std::string &df) {
...
inline std::tuple<std::string, ShapesStorage, std::vector<Tree>> parse_externalData(std::string &&data_asString) {
    return parse_externalData(data_asString);
}
...
inline std::tuple<std::string, ShapesStorage, std::vector<Tree>> parse_integratedData(std::string &df) {
    ...
    return parse_externalData(file_toString(df));
